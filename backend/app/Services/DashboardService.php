...
{
    public function summaryForCompany(int $companyId, ?User $user = null): array
    {
        ...
        if ($user !== null && ! $user->isAdmin()) {
            $projectQuery->whereHas('teamMembers', fn ($query) => $query->where('users.id', $user->id));
        }
        ...
        $recentProjectsQuery = Project::query()
            ->forCompany($companyId)
            ->with('client:id,name')
            ->orderByDesc('updated_at');

        if ($user !== null && ! $user->isAdmin()) {
            $recentProjectsQuery->whereHas('teamMembers', fn ($query) => $query->where('users.id', $user->id));
        }
